...
#import "./theme.typ"
#import "./layout.typ": kopfzeile, fusszeile, seitenzahl, seitenzahl-format, repeat
#import "./typo.typ": luecke, randnotiz
#import "./util.typ": place-label, decimal-fix
...
#let aufgaben-reset() = {
  __c-aufgaben.update(0)
  __c-punkte.update(0)
  __s-aufgaben.update(())
  __s-in_aufgabe.update(false)
  __s-in_teilaufgabe.update(false)
  __s-erwartungen.update(())
}

// update repeat layout with reset for counters
#let repeat = repeat.with(sep: () => {
  aufgaben-reset()
  pagebreak()
})
...
#let d_aufg(
  ...
  #if nr in (none, auto) {
    __c-aufgaben.display((..c) => numbering(format, c.pos().first()))
  } else {
    numbering(format, nr)
  }
]
#let d_punkte(
  ...
  if punkte == 1 [#punkte #singular] else [#punkte #plural]
}
...
#let punkte(nr: none, func: d_punkte) = {
  locate(loc => {
    ...
    func(
      __s-erwartungen.final(loc).filter(erw => erw.aufgabe == n).fold(0, (p, erw) => p + erw.punkte),
    )
  })
}
...
#let d_erwpunkte(aufg: none, teil: none, format: punkte => [#align(right)[_(#punkte.join(" + "))_]]) = {
  locate(loc => {
    ...
    let p = __s-erwartungen.final(loc).filter(erw => {
...
#let __d_loesung(aufg) = [
  #d_enum(
    numbering: "(1)",
    ..aufg.loesung.filter(l => l.teil == 0).map(l => l.body),
  )
  #let enums = ()
  #for t in range(aufg.teile) {
    enums.push(
      d_enum(
        numbering: "(1)",
        ..aufg.loesung.filter(l => l.teil == t + 1).map(l => l.body),
      ),
    )
  }
  #if enums.len() > 0 {
...
#let d_loesungen() = page(
  numbering: "I",
  header: kopfzeile(mitte: () => [Lösungen]),
...
#let aufgabe(
  ...
  if use {
    ...
    let ic = ""
    if icons != none {
      ic = randnotiz(offset: .2em)[#text(size: 0.88em)[#{
            (icons,).flatten().join()
          }]]
    }
    if page {
    ...
    if header [== #ic#d_aufg(nr: number)#if titel != none [#h(2em)#text(fill: theme.text.default)[#titel]]#h(1fr)#punkte(func: p => if p > 0 {
        text(fill: theme.secondary, size: 0.88em)[#d_punkte(p)]
      }) <aufgabe>]
    // if type(icons) != none {
    // 	icons = (icons,).flatten()
